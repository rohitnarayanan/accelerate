...
public abstract class AccelerateCache<K, V> implements Serializable {
	 ...
	 */
	@Autowired
	protected StaticListenerUtil staticListenerUtil = null;

	/**
	 * {@link Map} containing the cached information
	 */
	...
	public void initialize() {
		Assert.state(!this.initialized, "Cache already initialized");
		...
		try {
			...
			this.staticListenerUtil.notifyCacheLoad(AccelerateCache.this);

			this.initialized = true;
			LOGGER.info("Cache [{}] Initialized", name());
		} catch (Exception error) {
			methodError = error;
			AccelerateException.checkAndThrow(error, "Error in initializing cache [%s]", this.cacheName);
		} finally {
	 ...
	 *             {@link StaticListenerUtil#notifyCacheLoad(AccelerateCache)}
	 */
	@Log
	@ManagedOperation(description = "This method refreshes the cache")
	public void refresh() throws AccelerateException {
		Assert.state(this.initialized, "Cache not initialized");
		Assert.state(this.refreshable, "Cache not refreshable");
