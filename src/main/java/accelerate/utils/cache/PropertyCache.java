...
public class PropertyCache extends AccelerateCache<String, String> {
	 ...
	 *
	 * @param aPropertyKeys
	 *            array containing string to be concatenated to form the property
	 *            key
	 ...
	 * (non-Javadoc)
	 * 
	 * @see accelerate.utils.cache.AccelerateCache#loadCache(org.springframework.
	 * cache.Cache)
	 */
	/**
	 * @param aCacheMap
	 * @throws AccelerateException
	 *             thrown by
	 *             {@link PropertiesUtil#LoadProperties(org.springframework.context.ApplicationContext, String)}
	 */
	@Override
	protected void loadCache(final Map<String, String> aCacheMap) throws AccelerateException {
		...
		if (!isEmpty(getConfigURL())) {
			PropertiesUtil.LoadProperties(this.applicationContext, getConfigURL()).forEach((key, value) -> {
				String keyString = key.toString();
				if (keyString.startsWith(prefix)) {
					aCacheMap.put(keyString.substring(prefix.length()), (String) value);
				}
			});
		}
		...
		setCacheAge(aCacheMap.get("cacheAge"));
	}
